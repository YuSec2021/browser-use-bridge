...
class BrowserSession:
    ...
    async def switch_tab(self, tab_id: str) -> BrowserTab:
        ...
        previous_tab_id = self.session_manager.active_tab_id or ""
        tab = self.session_manager.set_active(tab_id)
        with contextlib.suppress(Exception):
            await tab.page.bring_to_front()
        await self.session_manager.refresh_tab(tab)
        self.event_bus.emit(
            TabSwitchedEvent(session=self, tab_id=tab.id, previous_tab_id=previous_tab_id, url=tab.url)
        )
        return tab
    ...
    async def _launch_browser_process(self) -> str:
        ...
        if user_data_dir is None:
            self._temp_profile = tempfile.TemporaryDirectory(prefix="browser-use-bridge-")
    ...
    async def _install_watchdogs(self) -> None:
        from browser_use_bridge.browser.watchdogs import CrashWatchdog, DownloadWatchdog, PopupWatchdog
