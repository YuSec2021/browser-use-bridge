...
from browser_use_bridge.mcp import CURRENT_STATE_URI, BrowserUseServer
...
def test_mcp_claude_config_json_points_to_stdio_command() -> None:
    ...
    server = payload["mcpServers"]["browser-use-bridge"]
    assert server["command"] == "browser-use-bridge"
...
def test_mcp_initialize_and_tools_list_contract() -> None:
    ...
    assert "browser-use-bridge" in initialize["result"]["serverInfo"]["name"]
